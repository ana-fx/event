...
import { Plus, Search, Edit2, Trash2, Shield, Store, ScanBarcode } from "lucide-react";
...
export default function UserListContent() {
    ...
    const getRoleBadge = (role: string) => {
        switch (role) {
            ...
            case 'reseller': return <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold bg-primary/10 text-primary dark:text-primary"><Store className="w-3 h-3" /> Reseller</span>;
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <Link
                    ...
                    className="px-4 py-2.5 bg-primary text-white font-bold rounded-lg hover:bg-primary transition-colors shadow-sm shadow-primary/20 disabled:opacity-70 flex items-center justify-center"
                >
            ...
            <div className="bg-(--card) p-4 rounded-xl border border-(--card-border) shadow-sm flex flex-col md:flex-row gap-4">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-2 rounded-lg border border-(--card-border) bg-(--background) text-(--foreground) focus:border-primary focus:ring-2 focus:ring-primary/20 dark:focus:ring-primary/20 outline-none transition-all placeholder:opacity-50"
                    />
                </div>
                                ...
                                <tr key={user.id} className="hover:bg-primary/5 transition-colors group">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2 text-(--foreground)">
                                            <Link href={`/admin/users/edit?id=${user.id}`} className="p-2 text-gray-400 hover:text-primary hover:bg-primary/10 rounded-lg transition-colors">
