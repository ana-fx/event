...
function EditEventForm() {
    ...
    useEffect(() => {
        ...
        const fetchEvent = async () => {
            try {
                ...
                // The API returns a list when filtered, but here we might need a specific endpoint or logic
                // Assuming get by ID returns the single object or we filter it from the list if the API is generic list
                // Based on backend implementation: `GetEvents` checks for `id` query param.

                // If the backend returns an array for ?id=, take the first one; if object, take it.
                ...
                setPreviews({
                    ...
                    organizerLogo: data.organizer_logo_path || "",
                });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            data.append("id", id!); // Backend expects ID for update, probably in URL or body. 
            // Standard PUT usually takes ID in URL. Let's check backend handler.
            // Backend `UpdateEvent` reads ID from query param `r.URL.Query().Get("id")` 
            // OR it might expect it in the body if `GetEvents` uses query.
            // Actually, standard REST is `PUT /events/{id}` or `PUT /events?id={id}`. 
            // Let's assume `PUT /admin/events?id={id}` is supported or we send it in body.
            // Checking previous logs: `UpdateEvent` uses `idStr := r.URL.Query().Get("id")`.
            ...
            // Fees (preserve existing or send defaults if needed, simpler to send defaults for now)
