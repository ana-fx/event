    ...
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center justify-between mb-8">
            <div>
                ...
                <h2 class="text-3xl font-bold text-gray-900">Edit Reseller</h2>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.resellers.update', $reseller) }}" method="POST" enctype="multipart/form-data"
                class="p-8">
                @csrf
                @method('PUT')
                ...
                <div class="flex items-center gap-6 pb-8 border-b border-gray-100 mb-8">
                    <div class="shrink-0 relative">
                        ...
                        @else
                            <div
                                class="w-24 h-24 bg-primary-100 text-primary-600 rounded-full flex items-center justify-center text-3xl font-bold ring-4 ring-gray-50">
                                {{ $reseller->initials() }}
                            </div>
                        @endif
                        <label for="photo"
                            class="absolute bottom-0 right-0 bg-white border border-gray-200 rounded-full p-1.5 shadow-sm cursor-pointer hover:bg-gray-50 transition-colors">
                    ...
                    <div>
                        <h3 class="text-xl font-bold text-gray-900">Reseller Photo</h3>
                        <p class="text-sm text-gray-500 mt-1">Reseller avatar. Max 1MB.</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div class="md:col-span-1">
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $reseller->email) }}"
                            required
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                    </div>
                    ...
                    <div class="md:col-span-1">
                        ...
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $reseller->phone) }}"
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="bio" id="bio" rows="3"
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">{{ old('bio', $reseller->bio) }}</textarea>
                    </div>

                    <div class="md:col-span-2">
                        <label for="address" class="block text-sm font-medium text-gray-700 mb-2">Address</label>
                        <textarea name="address" id="address" rows="2"
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">{{ old('address', $reseller->address) }}</textarea>
                    </div>
