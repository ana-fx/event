<x-layouts.admin>
    <div x-data="{ assignModalOpen: false }">
        <div class="flex items-center justify-between mb-8">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">Event Resellers</h2>
                <p class="text-sm text-gray-500">Manage resellers for event: <span
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($event->resellers as $index => $reseller)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <form
                                        action="{{ route('admin.events.unassign-reseller', ['event' => $event, 'reseller' => $reseller]) }}"
                                        method="POST" class="inline"
                                        onsubmit="return confirm('Are you sure you want to remove this reseller from the event?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"
                                            title="Unassign Reseller">
        ...
        <div x-show="assignModalOpen" style="display: none;"
            ...
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative"
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-4">Assign Reseller</h3>

                <form action="{{ route('admin.events.assign-reseller', $event) }}" method="POST">
                    @csrf
                    <div class="mb-4">
                        <label for="reseller_id" class="block text-sm font-medium text-gray-700 mb-1">Select
                            Reseller</label>
                        <div class="relative" x-data="{
                            open: false,
                            selected: '{{ $availableResellers->first()->id ?? '' }}',
                            label: '{{ $availableResellers->first() ? $availableResellers->first()->name : 'No available resellers' }}'
                        }">
                            ...
                            <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
                                ...
                                <div class="py-1 max-h-48 overflow-y-auto">
                                    @forelse($availableResellers as $reseller)
                                    ...
                                    @empty
                                        <div class="px-5 py-3 text-sm text-gray-400">No available resellers found</div>
                                    @endforelse
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex justify-end">
                        <button type="submit"
                            ...
                            @if($availableResellers->isEmpty()) disabled @endif>
                            Assign
                        </button>
                    </div>
                </form>
