    ...
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center justify-between mb-8">
            <div>
                ...
                <h2 class="text-3xl font-bold text-gray-900">Edit Admin</h2>
            </div>
        </div>
                ...
                <div class="flex items-center gap-6 pb-8 border-b border-gray-100 mb-8">
                    <div class="shrink-0 relative">
                        @if($admin->profile_photo_path)
                            <img src="{{ Str::startsWith($admin->profile_photo_path, ['http', 'https']) ? $admin->profile_photo_path : (file_exists(public_path($admin->profile_photo_path)) ? asset($admin->profile_photo_path) : asset('storage/' . $admin->profile_photo_path)) }}" alt="{{ $admin->name }}"
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-1">Admin avatar. Max 1MB.</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="bio" id="bio" rows="3"
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">{{ old('bio', $admin->bio) }}</textarea>
                    </div>

                    <div class="md:col-span-2">
                        <label for="address" class="block text-sm font-medium text-gray-700 mb-2">Address</label>
                        <textarea name="address" id="address" rows="2"
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">{{ old('address', $admin->address) }}</textarea>
                    </div>
